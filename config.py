...
# 画像キャッシュ設定
# 【2026-09の実機トラブルから追加】以前は「表示対象の全画像」を起動時・変更検知時に
# まとめてデコード・画面サイズへスケーリングした状態で常駐メモリーに保持していた。
# 画像の枚数が多い・解像度が高いと、この常駐メモリーがPi Zero 2W（RAM 512MB）を
# 圧迫する一因になり得ることが分かったため、「固定表示（📌）の画像だけは常駐、
# それ以外の通常画像は表示中+数枚先までのウィンドウだけを保持する」方式に変更した。
# 一覧の枚数・スクロール方向は既に分かっているため、表示が追いつく範囲で先読みしておけば、
# 切り替え時の体感速度は落とさずに常駐メモリーを抑えられる。
IMAGE_PREFETCH_WINDOW = 3     # 通常画像を、現在表示中に加えて何枚先まで先読みキャッシュしておくか
                               # （固定表示画像は常に全て常駐するため、ここには含まれない）。
                               # Web設定画面・USB設定ファイルから変更可能（1〜10の範囲）

# 固定表示は常に全件デコード済みでメモリーに常駐する設計のため、上限を設けないと
# 大量に固定表示を設定した場合（特に高解像度運用時）にメモリーを圧迫し得る。
# 上限に達した状態でこれ以上「固定表示」に設定しようとした場合は拒否され、
# 既存の固定表示はそのまま維持される（自動で誰かを外すことはしない）。
# Web設定画面・USB設定ファイルから変更可能（1〜20の範囲）
MAX_PINNED_IMAGES = 10        # 固定表示に設定できる画像の上限枚数

# 固定表示のブロック表示設定
# 既定（PINNED_BLOCK_SIZE=0）では、固定表示画像は特別な並べ方をされず、
# 通常画像の並び（ファイル名順）に混在したまま表示される（従来通りの挙動）。
# PINNED_BLOCK_SIZEを1以上に設定すると、「固定表示をN枚まとめて→通常画像を
# M枚まとめて→また固定表示N枚→…」というブロック単位の交互表示に切り替わる。
# 例）固定表示A〜H(8枚)・通常表示1〜9(9枚)・固定枚数3・通常枚数4の場合:
#   A,B,C,1,2,3,4,D,E,F,5,6,7,8,G,H,A,9,1,2,3,B,C,D,4,5,6,7,...
# （固定表示・通常表示それぞれ独立したカーソルで末尾まで来たら先頭に循環する。
# 優先表示（PRIORITY_INTERVAL）は、この「通常画像」パート内で従来通り
# 割り込み表示される＝両方の機能を併用できる）。
PINNED_BLOCK_SIZE = 0         # 1ブロックあたり連続表示する固定表示画像の枚数（0=無効、既定）
NORMAL_BLOCK_SIZE = 4         # 1ブロックあたり連続表示する通常画像の枚数（PINNED_BLOCK_SIZE>0の時のみ有効）
...
# 2通りの起動経路がある：
#   A. Web設定画面から（PCからブラウザでアクセスしている場合）
#      画面の「今すぐUSBへ書き出す」ボタンを押すだけで、その場でUSBメモリーへ
#      書き出す。Web設定画面に入れていること自体を許可の根拠とするため、
#      合言葉ファイルは不要（詳細はpophug-usb-importのmanual_export()）。
#   B. 本体ボタンの長押し（PCレス運用時）
#      1. 本体ボタンをEXPORT_HOLD_SECONDS秒以上長押しして「書き出し待受けモード」に入る
#      2. 待受け中に挿したUSBメモリーの直下に、この機体のホスト名が1行として
#         書かれたテキストファイル（EXPORT_KEY_FILENAME）が置かれていた場合だけ、
#         実際に書き出しを行う。ここでの「合言葉」は秘密の値ではなく、単に
#         「このUSBでの書き出しを許可された機体のホスト名一覧」を意味する
#         （機密性を高める必要は無いという前提の設計）。1つのファイルに複数の
#         ホスト名を1行ずつ書いておけば、そのUSB1本を複数のpophug本体に順番に
#         挿すだけで、それぞれの機体から書き出せる（マスターUSB運用）。
#      ホスト名が書かれていない・ファイル自体が無い場合は書き出しは行われず、
#      通常のUSB取り込み動作だけが行われる（待受け状態はそのまま残り、
#      USBを挿し直して再挑戦できる）。誤って画像を抜かれることは無い。
...
EXPORT_STANDBY_TIMEOUT_SECONDS = 180  # 待受けモードのままUSBが挿されなかった場合に
                                       # 自動キャンセルするまでの秒数
EXPORT_KEY_FILENAME = "pophug-export-key.txt"  # USB直下に置く、許可ホスト名一覧ファイル名
                                                # （大文字小文字は区別しない）
