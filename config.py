...
# スタンドアロンモード設定
# 外部Wi-Fiが存在しない屋外（公園・催事場など）での移動販売を想定。
...
STANDALONE_CHECK_INTERVAL = 20       # 既知のWi-Fiに繋がっているか再判定する間隔（秒）
STANDALONE_BOOT_GRACE_SECONDS = 25   # 起動直後、既知のWi-Fiへの接続を待つ猶予（秒）

# スタンドアロンモード設定
# 外部Wi-Fiが一切無い屋外（公園・運動公園など）での移動販売を想定。
# 起動時に知っているWi-Fiへ接続できるか確認し、接続できなければ自分専用の
# アクセスポイントを常時起動する（画面は通常のスライドショーのまま、裏側で待機）。
# スマホは繋ぎたい時だけ接続すればよいため、スマホ側のバッテリー消費を抑えられる。
STANDALONE_AUTO_ENABLED = True       # 自動判定を有効にするか
