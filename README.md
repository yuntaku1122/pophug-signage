...
| `pophug-usb-import` | USBメモリー挿入時に自動実行される、root権限が必要な画像取り込み・オフラインアップデートパッケージの検出/ステージング・設定ファイルの自動適用処理（要インストール、後述） |
...
## QRコードボタン（5段階の長押しに対応、USBアップデート確定待ち中は専用モードに切替）

1つのボタンで5つの操作ができる。**判定は「離した瞬間の合計長押し時間」**で行われ、
...
キャンセル（閉じる）操作になる。「表示リセット」は次の事業者へ本体を引き継ぐ場面を
想定した機能で、実行後は固定表示画像だけが表示された状態になり、次の事業者が
自分のUSBメモリーを挿すと、そのUSBの中身がそのまま表示されるようになる。

**USBアップデートパッケージが確定待ちの間**は、上記の5段階ラダーは一時的に丸ごと
...
**Wキー**でWi-Fiセットアップモード、**Rキー**で表示リセット、**Sキー**でシャットダウンを
それぞれ即座に確認できる（実機と違い長押し判定は無い）。
