...
## 複数台セットアップ時のホスト名の一意化・SSH鍵の個別化

SDカードは毎回同じイメージ（ホスト名`pophug`）を書き込むだけでよい。初回起動時に
自動的にMACアドレス下4桁を使って`pophug-XXXX`のような一意なホスト名に変更されるため、
複数台を同じネットワークに繋いでも`.local`名が衝突しない。SSHホスト鍵が存在しない
場合（後述のマスターイメージから複製した直後）は、その場で新規に生成し直すため、
全台が同じSSH鍵を共有してしまうこともない。
...
インターネット接続すら不要）。起動すればホスト名の一意化・SSH鍵の個別化・Wi-Fiセットアップ
モードのボタン操作まで、全て最初から使える状態になる。

### 手順

1. **お手本にする1台で、機体固有の情報をリセットする**（この後そのPiは電源を切るだけで、
   普段使いには戻さないこと）
   ...
   ホスト名を`pophug`に戻す、SSHホスト鍵を削除、`machine-id`を消去、保存済みのWi-Fi接続を
   削除、`images/`フォルダの中身（写真・状態ファイル）を消去、といった処理を自動で行う。
   最後に案内される通り`sudo shutdown -h now`で電源を切り、SDカードを取り出す。
