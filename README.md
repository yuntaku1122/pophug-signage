...
1つのボタンで3つの操作ができる。**判定は「離した瞬間の合計長押し時間」**で行われ、
押している間は画面に「あと何秒で何が起きるか」の進捗が表示される。

| 操作 | 動作 |
|---|---|
| 短押し | アップロードページのQRコードを表示/非表示トグル |
| `WIFI_SETUP_HOLD_SECONDS`秒（デフォルト3秒）以上長押し | Wi-Fiセットアップモードに入る |
| `SHUTDOWN_HOLD_SECONDS`秒（デフォルト8秒）以上長押し | シャットダウン |

Wi-Fiセットアップモード中にボタンを押すと、押した時間に関わらずキャンセル操作になる。

GPIOが無い環境（Mac等）では、ウィンドウ選択中に**Qキー**でQR表示、**Wキー**でWi-Fiセットアップモード、
**Sキー**でシャットダウンをそれぞれ即座に確認できる（実機と違い長押し判定は無い）。
