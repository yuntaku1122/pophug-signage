...
| バージョン | 内容 |
|---|---|
| 4.34.0 | 【新機能】Web設定画面の「アップデート」欄に過去のアップデート結果履歴（直近10件、日時・バージョン・成功/失敗・失敗理由）を表示。オンラインOTA・USBオフラインアップデート両方の結果を記録し、SSH無しでもブラウザタブを開き直すだけで確認できるように |
| 4.33.1 | USBに写真が1枚も無い場合の全画面通知を、「完全に空」だけでなく「アップデートのみ」「設定ファイルのみ」見つかった場合も同じ重みでフルスクリーン表示するよう統一。アップデート検出時の重要な案内が見落とされないように |
