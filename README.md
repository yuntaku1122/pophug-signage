...
sudo cp pophug-signage.service /etc/systemd/system/
...
`pophug-signage.service`は本体（main.py）を起動するユニット。以降のバージョンアップでは
`pophug-netctl`・`pophug-hostname-setup`関連と同様にOTAアップデートで自動的に配信・反映
されるため、手動で編集し直す必要は無い（内容を変えたい場合はリポジトリ側の
`pophug-signage.service`を直接編集してリリースする）。
...
**この機能は`pophug-signage.service`（`/etc/systemd/system/`配下）の`[Service]`セクションの
以下の設定によって有効化されている。リポジトリの`pophug-signage.service`に含まれており、
OTAアップデートで自動的に配信・反映されるため、通常は手動編集不要。**
...
- `Restart=on-failure`: ウォッチドッグによるkillも含め、異常終了時に自動再起動する
  ...
  ここでのRestartとは競合しない）
- 設定を変更したい場合は、リポジトリの`pophug-signage.service`を直接編集してリリースすれば、
  次回のOTAアップデート適用時に各機体へ自動的に反映される
...
| バージョン | 内容 |
|---|---|
| 4.15.0 | Wi-Fi接続の「非表示ネットワーク」指定を自動判定化（チェックボックス廃止）。Wi-Fi設定画面に現在の接続状態（SSID・IPアドレス）を表示するようにした |
| 4.14.4 | アップロードサーバーの待ち受けをIPv4専用(0.0.0.0)からデュアルスタック(::)に変更。IPv4が使えずIPv6のみの端末からもアクセスできるようにした |
| 4.14.3 | Wi-Fi設定画面のパスワード入力欄に「パスワードを表示する」チェックボックスを追加 |
| 4.14.2 | v4.14.0で常時付与していた`hidden yes`が通常のWi-Fi接続を不安定にする副作用を修正。非表示ネットワークはチェックボックスで明示指定する方式に変更 |
