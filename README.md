...
**v4.27.0以降、この一式はOTAアップデート時に自動配置されます**（pophug-update-applyが
自動でインストール・daemon-reload・udevルール再読み込みまで行う。exfatprogsも
未インストール時のみバックグラウンドで自動インストールされる）。以下の手動手順が
必要なのは、次のいずれかに該当する場合のみです。

- **v4.27.0より前のバージョンから初めてアップデートする機体**（自動化ロジック自体が
  まだ入っていない旧バージョンのpophug-update-applyが動くため、その1回だけ自動化されない）
  → この場合は、通常のOTAアップデート実行後に、以下を1回だけ手動実行すれば十分です
    （OTAで既に新しいpophug-update-applyに置き換わっているため、これを直接呼び出すだけで
    USB機能一式が自動配置されます）。
    ```bash
    sudo /usr/local/bin/pophug-update-apply /home/pophug/pophug-signage
    ```
- **量産マスターイメージ作成時など、OTAを経由せずゼロから手動セットアップする場合**
  → 以下をフルで実行してください。
